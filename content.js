...
const DIALOGUE_CUES = [
  ...
  ["When do you need my comments?", "Why is the timing important?"], ["Can we still deliver on the original date?", "What alternative would protect the work?"], ["How can I help with the project?", "When should I flag any resource needs?"], ["What did you think of the presentation?", "What is the one change you would make?"], ["How should we divide the work?", "Do you have any feedback on my draft?"],
  ["What changed in the latest quarter?", "How confident are we in the result?"], ["What may have caused the decline?", "Could anything else have influenced it?"], ["What is the trade-off with the faster option?", "How should we evaluate it?"], ["Which option did we choose?", "What happens next?"], ["How do you read the results?", "What decision have we made?"],
  ["Can we still hit the date with the current scope?", "How should we set feature priorities?"], ["Is there anything customers should know?", "What is the team doing about it?"], ["What should we understand before proposing a solution?", "What would you recommend based on that?"], ["When should the regional teams meet?", "How can we invite different perspectives?"], ["How can we protect the deadline?", "When will the customer receive the revised plan?"],
...
function enrichDay(item) {
  ...
  item.dialogue = item.dialogue || [
    { speaker: "Colleague", text: cues[0] },
    { speaker: "You", text: item.sentences[0].text },
...
window.GBME_CONTENT.days.forEach(enrichDay);
